...
from tcip_mcp.tools.project_tools import read_datasets, register_dataset  # noqa: E402
...
def restamp(dataset_root: str, project_root: str | None = None) -> int:
    ...
    matches = [r for r in read_datasets(proj) if r.get("id") == ds_id]
    ...
    written_identity = require_dataset_identity(root)
    written_fp = written_identity.get("fingerprint")
    written_matches = [r for r in read_datasets(proj) if r.get("id") == ds_id]
