...
|------|-------|-----------------|
| `model_source` | `pipelines.model_build.build_model` | `{"builder": "my_module:build_net", "builder_kwargs": {...}, "source_files": [...], "task": "detection", "in_chans": 3, "image_stats_sampling": {...}}`: your `nn.Module`. `image_stats_sampling` is required alongside `builder_kwargs.image_mean`/`image_std`: it records which images (or, for a sampled statistic, which pixel windows and seed) they were derived from, so the provenance travels beside the kwargs rather than inside them. `preflight_config(smoke=True)` (run by `launch_training`) builds + smokes it at the resolved dims before the training subprocess spawns. |
...
| audited sinks | `ctx.log_metrics`, `ctx.save_checkpoint` (stamps kind + `model_source` + `experiment_id`), `ctx.record_artifact`, `ctx.should_cancel`, `ctx.set_final_weights` (declares the deliverable checkpoint), `ctx.report_objective` (reports HPO trial progress for pruning, no-op outside HPO) |
