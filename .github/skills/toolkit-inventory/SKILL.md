...
| audited sinks | `ctx.log_metrics`, `ctx.save_checkpoint` (stamps kind + `model_source` + `experiment_id`; refuses a payload with no `model_state_dict`, since that stamped kind is what a predictor sniffs to load the weights), `ctx.record_artifact`, `ctx.should_cancel`, `ctx.set_final_weights` (declares the deliverable checkpoint), `ctx.report_objective` (reports HPO trial progress for pruning, no-op outside HPO) |
