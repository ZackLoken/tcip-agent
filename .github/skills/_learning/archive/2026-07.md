# Learning journal — archive (2026-07)

Graduated entries moved out of `journal.md` per the `self-improvement` skill's **Prune** step:
each was distilled into a durable artifact (a skill, a `CLAUDE.md` rule, a tool, a config fix)
and is kept here verbatim as the historical record. The live buffer is `../journal.md`.

Some detail below is intentionally **superseded** — e.g. the 2026-07-09 ruling that
`catkin_elongation_date` = onset was later reversed (L-SKILL-1, 2026-07-15). Trust the durable
artifact and the current journal, not an archived narrative.
...
### L-SKILL-1 — RESOLVED (Zack, 2026-07-15): `catkin_elongation_date` = the 95% majority crossing
`crops.yml` is immutable truth (built from the breeders' CSV) and stays "Date when **most** catkins
have elongated". Zack ruled: that trait ≈ `catkin_95per_date` (his best guess, to be confirmed with
the breeders), superseding the 2026-07-09 onset ruling. `phenology.plant_milestones` now sets
`catkin_elongation_date = catkin_95per_date` (was `elongation_onset_date`); `elongation_onset_date`
is kept as a **separate** helper (first date any elongation), not the delivered trait. Flagged
**provisional** in the phenology skill + code — re-map if the breeders rule otherwise; do not deliver
the milestone as validated until the classifier + operating point pass the existing held-out gate.
Docstrings/tests/web-route/smoke updated to the 95% definition.
...
### L-SKILL-3-residual — RESOLVED (2026-07-16 skills+docs refresh): the two deferred edits, applied
**Observation (2026-07-14).** Two non-core skill body edits were noted on the skills-rebuild pass but
not applied:
...
**Resolution.** A docs-lens audit re-flagged both independently (plus Pattern F, which has the same
issue — relational/contextual ranking is future work per README's Roadmap). Applied: Patterns C, E,
and F in `pipeline-design/SKILL.md` now carry an "Out of current build scope" callout; the
`model_spec` example (with corrected `anchor_detection`/`focal` component names — the old
`detection_head`/`focal_loss` names don't exist in the registry and would raise `KeyError`) is now
canonical in `pipeline-design/SKILL.md` only, and `training/SKILL.md` references it instead of
repeating it.
