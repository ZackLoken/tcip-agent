...
def resolve_pixel_size(source: Path | BandGroupRef) -> tuple[PixelSize | None, str]:
    ...
    6. Every axis reports the same ``unit_code`` from ``{"9001", "9002", "9003"}`` (meter, foot, US
      survey foot); the factor to meters is that axis's own ``unit_conversion_factor``.
    7. ``pixel_scale_x`` and ``pixel_scale_y`` are both positive.
    ...
    if len(codes) != 1 or next(iter(codes)) not in _UNIT_CODES_TO_METERS:
        return None, f"its CRS EPSG:{gt.epsg}'s units are not meter, foot or US survey foot"
