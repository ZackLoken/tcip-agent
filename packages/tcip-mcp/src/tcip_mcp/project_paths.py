...
Durable platform state, the platform's own audit log, the experiment store, and the model registry,
anchors here so a whole project is self-contained under one ``<root>/.tcip/``.
...
A long-lived process binds the variable once at startup, through :func:`pin_platform_root`. A
process that opts in (``from_marker=True``) binds from the workspace's active-project marker when
...
def require_and_pin_platform_root(explicit: str | None) -> Path:
    ...
    root. Pins the environment variable before the caller imports or calls its tool function, so
    that resolution and every later one in the process, including a store bound after this call,
    land under the platform state root.
    """
...
def resolve_output_path(path: "str | Path") -> Path:
    ...
    An absolute path is the caller's own explicit choice and is returned unchanged. A relative path
    resolves against :func:`platform_state_root`, never the process cwd.
    """
    p = Path(path)
...
def resolve_state(path: Path) -> Path:
    ...
    - A relative ``path`` is prefixed with ``$TCIP_STATE_ROOT`` when pinned, so a process launched
      from a subdir still writes to the one platform ``.tcip/``.
    - When unpinned, a relative ``path`` is returned as-is → resolved against the current directory
      at use, so each caller's own cwd decides where it lands.
    """
    if path.is_absolute():
        return path
    override = os.environ.get(ENV_VAR)
...
def resolve_state_or(path: Path, fallback: Path) -> Path:
    ...
    ``fallback`` rather than ``path`` unchanged.
    """
...
class RootBinding:
    """How a process's platform-state root was decided by :func:`pin_platform_root`, or set since
    by :func:`repin_platform_root`.

    ``source`` is ``"marker"`` (the workspace's active-project marker supplied the root),
    ``"inherited"`` (the process kept a root it was launched with), ``"repo_root"`` (neither was
    available), or ``"adopted"`` (a later :func:`repin_platform_root` call, an explicit adopt
    rather than the startup bind). ``inherited_root`` is the variable's value before this bind,
    ``None`` when nothing was inherited; for an ``"adopted"`` binding it is the root this process
    was pinned to just before the repin. ``marker_problem`` is ``None`` when the process either did
    not consult the marker or the marker named no project; otherwise the text of why the marker
    could not be used: a store refusal, a lock timeout, or a name
    :func:`tcip_mcp.workspace.adoptable_project_root` refuses to open. Always ``None`` on an
    ``"adopted"`` binding.
    """
...
def root_binding() -> Optional[RootBinding]:
    """This process's :class:`RootBinding`, or ``None`` before either :func:`pin_platform_root` or
    :func:`repin_platform_root` has run.
    """
    return _binding
...
def pin_platform_root(*, from_marker: bool) -> RootBinding:
    ...
    ``from_marker=True``: the workspace's active-project marker's project, when
    :func:`tcip_mcp.workspace.active_project_if_present` (with ``create=False``) finds one
    adoptable; otherwise the inherited variable, when set; otherwise :func:`repo_root_from_here`. A
    store refusal, a lock timeout, or a marker naming a project that is not adoptable is caught
    into the returned binding's ``marker_problem`` rather than raised.

    ``from_marker=False``: the inherited variable, else the repo root, recorded in the returned
    binding rather than only applied.

    Call once, after the process has bound a storage backend (a marker read needs one) and before
    anything resolves a ``.tcip`` path. Returns the :class:`RootBinding`, which
