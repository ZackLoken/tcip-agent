...
| `dataset_source` | `pipelines.data.datasets.build_from_dataset_source` | `{"builder": "my_module:build_ds", "builder_kwargs": {...}, "source_files": [...], "task": "..."}`: a bespoke `Dataset`, for a task the known loaders don't cover or for one they do. Mirrors `model_source`; the known loaders stay the default. Your builder receives `samples` (the sample list the platform's own producer named for the side being built), `id_map` (the class map they were admitted under, `None` where the ground-truth shape carries its own classes), `task` and `transforms`, and never a directory, a document path or a format flag: the platform names the samples and you build over them. `builder_kwargs` configure your builder and may not restate those four, and the factory refuses any kwarg beyond them. A task with no built-in loader is not a task with no producer: the platform admits by the shape of the ground truth `data.labels_dir` points at, whatever the task, so your builder receives the same samples a built-in loader would. |
