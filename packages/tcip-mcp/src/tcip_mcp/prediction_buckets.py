"""Prediction-bucket immutability: never silently overwrite predictions a human reviewed, and never
silently publish a second run into a bucket a prior run already filled.

A bucket is a directory (or set of task directories) holding per-image ``<stem>.json`` prediction
files, its identity the bucket directory's own path (relative to the dataset root when it sits
under one, its own resolved path otherwise, per :func:`bucket_key_of`). Once a reviewer has
recorded verdicts (accept/reject/edit) against any of a bucket's images, re-running inference or
re-staging into it would orphan those verdicts. So the prediction writers resolve a run-scoped
bucket through here: with verdicts (or, for a caller counting review state, any finished image)
present the default writes are redirected to the next free ``<name>@r2`` / ``@r3`` variant, and an
explicit ``overwrite=True`` is refused with a count.

:func:`resolve_writable_bucket`'s ``refuse_documents`` refuses a requested bucket that already
holds prediction documents, with no verdict yet recorded against it, outright rather than
redirecting or overwriting. ``count_review_state`` widens what the guard counts from detection
verdicts alone to every image a reviewer has finished, a bulk accept included.
...
(:func:`~tcip_mcp.tools.proposal_tools.stage_proposals`).

Exhaustion is refused for every caller: when every ``<name>@r<n>`` variant up to the search's
ceiling is already taken under the reading that caller asked for, the resolver raises
:class:`BucketHasVerdicts` naming no suggestion.

A bucket's documents leave it through
:func:`~tcip_mcp.tools.inference_tools.clear_prediction_bucket`, which moves a terminal
experiment's own recorded bucket into a dated archive so the path re-publishes. A bucket emptied by
hand, outside that door, is invisible to every guard here.
"""
...
def bucket_content_digest(*dirs: Path | str, memo: dict[str, str] | None = None) -> str:
    ...
    :func:`~tcip_mcp.pipelines.resolution.dataset_hash` over each directory's enumerated stems
    (:func:`bucket_stems`, sidecars excluded), so a replaced file, an added one and a deleted one
    all change the digest. Several directories (a prediction bucket's detect and segment dirs)
    combine in sorted directory order, so the caller's argument order does not matter.

    ``memo`` is a caller-owned dict, keyed by resolved directory, that lives for the span of one
    delivery: a bucket read several times inside one delivery is hashed once, and a call that
    passes a fresh dict (or none) reads every prediction file again. There is no cross-call cache.
    """
    if not dirs:
...
def bucket_stems_digest(*dirs: Path | str, images_dir: Path | str) -> str:
    ...
    A stem ``images_dir`` cannot resolve is refused by name. Several directories combine in sorted
    order, so the caller's argument order does not matter.
    """
    if not dirs:
...
def bucket_key_of(bucket_dir: str | Path | None) -> str:
    ...
    A bucket inside a dataset is named by its path relative to that dataset's root, so verdicts
    follow a dataset that moves. A directory under no dataset root is named by its own resolved
    path. No directory at all is :data:`~tcip_annotation.review_engine.NO_BUCKET`, the key for a
...
def _open_review_engine(review_state_dir: Path | str):
    """The review engine over ``review_state_dir``, or ``None`` when its store holds no verdicts at
    all: no engine is created for a never-reviewed dataset.
    """
    import tcip_store
...
def review_state_count(
    ...
    whose prediction document has since been removed. A ``not_started`` shard never counts. 0 when
    the store holds no verdicts at all.

    With ``names`` omitted (``None``), bucket-wide: every image name the engine holds state for
    under ``bucket``, not only the stems a document exists for today.

    With ``names`` given (identity against ``None``, never truthiness, so an empty ``()`` counts
    nothing), stem-scoped: only the images whose ``Path(img_name).stem`` is among ``{Path(n).stem
    for n in names}``, the rule
    :meth:`~tcip_annotation.review_engine.ReviewEngine.verdict_count_for_images` applies; a stem
    carrying a dot (e.g. ``plot_3.east``) misses in both counts the same way.
    """
    engine = _open_review_engine(review_state_dir)
    if engine is None:
        return 0
...
class BucketHasVerdicts(Exception):
    ...
    search's ceiling taken. ``unit`` names what ``count`` counts in the message: ``"review
    verdict(s)"`` (detection entries alone) by default, or ``"reviewed image(s)"`` under
    ``count_review_state`` (a detection verdict or a bulk accept alike).
    """
...
def resolve_writable_bucket(
    ...
    *,
    ...
    (detection verdicts alone, or every finished image under ``count_review_state``) and, with
    ``refuse_documents``, prediction-document immutability.

    ``dirs_for(name)`` returns the task dir(s) of the bucket variant ``name`` (one for a single-dir
    bucket, detect+segment for a prediction bucket). ``review_state_dir`` is ``None`` for a bucket
    under no dataset root, whose guard answers zero without touching disk; the document guard runs
    regardless.

    ``count_review_state`` (default off) widens the guard's own count from detection verdicts alone
    to every image a reviewer has finished, a bulk accept included (:func:`review_state_count`'s
    stem-scoped form).

    With the guard's count zero on the requested bucket: used as-is, unless ``refuse_documents`` is
    set and the bucket already holds a prediction document, which raises
    :class:`BucketHoldsDocuments` whatever ``overwrite`` says, naming the document count and the
    suggested first variant free of both the guard's count and a document (or ``None``). With a
    non-zero count on the requested bucket: that check runs first, ahead of any document check.
    ``overwrite=False`` (default) picks the next ``<requested>@r2`` / ``@r3`` variant free of the
    guard's count (and, with ``refuse_documents``, also free of a document); ``overwrite=True``
    raises :class:`BucketHasVerdicts`. When no variant up to ``max_variants`` is free, the resolver
    raises the class it was resolving for with ``suggested=None`` and a message saying every
    variant is taken, whether or not ``refuse_documents`` is set.

    A raster pass' own progress records, kept under a bucket's ``<out>/.tcip/`` subdirectory, never
    register as a document: :func:`bucket_document_stem_count` is a non-recursive ``*.json``
    listing with sidecars excluded.

    The check runs at resolution, so two doors racing into one bucket can each resolve it clean and
    interleave their writes. A documentless bucket answers zero here regardless of the review state
    its store still holds.
    """
...
def resolve_prediction_bucket(
    ...
    *,
    ...
    The path follows ``dataset_layout.prediction_dir``, and a requested bucket carrying review
    state under the caller's reading redirects on the model segment
    (``predictions/<model>@r2/<date>``), never the date. ``refuse_documents`` and
    ``count_review_state`` both forward to :func:`resolve_writable_bucket`.
...
def stage_prediction_shapes(
    ...
    *,
    ...
    A bucket that carries review state (a detection verdict or a bulk accept alike) is never
    overwritten (the default redirects to the next free ``<name>@r2`` variant; ``overwrite=True``
    raises :class:`BucketHasVerdicts`). ``annotations`` are name-based prediction records (subject
    + geometry + score) already stamped with their producer; boxes and polygons live in the one
    per-image file. Leaves ``refuse_documents`` off, since a staging bucket accumulates one stem
    per call, and turns ``count_review_state`` on.

    This door writes no stamp of its own, so a bucket holding nothing but staged shapes carries
    none. A bucket a producer already stamped keeps that producer's stamp.

    Returns the bucket actually written and the path; ``verdict_count`` counts reviewed images (a
    detection verdict or a bulk accept alike), not detection entries.
    """
