...
class ReviewPanel extends webview_base_1.WebviewPanelProvider {
    constructor(extensionUri) {
        super(extensionUri, "tcip-review", "TCIP: Review");
    }
    getHtmlContent(webview, nonce, sharedCssUri, sharedJsUri) {
        const reviewJsUri = (0, webview_base_1.getUri)(webview, this.extensionUri, "media", "review.js");
...
<body>
  ...
  <script nonce="${nonce}" src="${reviewJsUri}"></script>
     ...
     */
    showPredictions(imagePath, predictions) {
        if (!this.panel) {
            return;
        }
    ...
    handleMessage(msg) {
        switch (msg.type) {
            ...
            case "open_in_annotation": {
                vscode.commands.executeCommand("tcip-agent.openInAnnotation", msg.path);
                break;
            }
            case "request_image": {
     ...
     */
    handleRequestMatches(msg) {
        const imagePath = msg.path;
        const iouThreshold = msg.iouThreshold ?? 0.5;
        const confThreshold = msg.confThreshold ?? 0.25;
        const script = `
import json, sys
...
`;
    ...
    handleSaveReviewState(msg) {
        ...
        vscode.workspace.fs.readFile(uri).then((data) => {
            try {
                const existing = JSON.parse(Buffer.from(data).toString("utf-8"));
    ...
    handleDeleteGt(msg) {
        const imagePath = msg.imagePath;
        const lineIndex = msg.lineIndex;
        const labelType = msg.labelType ?? "detect";
        if (!imagePath || lineIndex == null) {
            return;
        }
        const dir = path.dirname(path.dirname(imagePath));
