...
describe("TuningTab sweep row actions", () => {
  ...
  it("renders the not-relaunchable reason only in the expanded region", async () => {
    ...
    render(<TuningTab />);
    const row = await screen.findByText("hpo-noreason");
    expect(screen.queryByText("this sweep's record holds no base config")).not.toBeInTheDocument();
  ...
  it("shows the stop-requested state after a cancel, until the sweep reads cancelled", async () => {
    vi.spyOn(tuningApi, "listSweeps").mockResolvedValue({
