"""The view-coverage record's viewing-context models. Every model here forbids an undeclared key."""
...
class WorkingScale(BaseModel):
    ...
    subject (``coverage_grid_zoom``), never derived from annotation content. ``zoom`` is the grid
    zoom the breeder set; ``source`` states who set it and when ("set by user:<name> at <time>").
    """
...
class CoverageViewing(BaseModel):
    ...
    ``bands`` and ``stretch`` are the two keys a plain RGB view (no composite selected)
    legitimately omits. The other four are always present, ``None`` where nothing applies:
    ``display_bounds`` is positional against the displayed bands, in the same order ``bands`` names
    them, and a one-element list is one bound applied to every displayed band. Each low/high value
    in a pair is a finite number or ``null``; a ``null`` bound means the stretch found no finite
    value for that half of that band's range (a raster whose sampled or served pixels held a NaN or
    an infinity). A plain view of a non-``uint8`` raster, and a band composite under
    ``stretch="none"``, both report the low as always zero.
    """
...
class CoverageRecord(BaseModel):
    """The stored per-image view-coverage record.
    ...
    one, been fully on screen at or above it. Whether a cell counts as "swept" is never stored
    here.
    """
