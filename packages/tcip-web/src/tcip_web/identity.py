...
along on each save/review request (``user`` field). When a request omits it, we fall back to an env
override then the OS login. ``user_id`` applies the platform convention that humans are
``user:<name>`` while tool producers stay bare (``sam``, ``baseline``).
"""
