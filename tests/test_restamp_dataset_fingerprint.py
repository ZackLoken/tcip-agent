...
from tcip_mcp.dataset_layout import (
    ...
    read_dataset_identity_document,
...
def test_read_dataset_identity_document_admits_the_bare_value_the_refusing_reader_rejects(
    tmp_path,
):
    """The raw layer this script's own reads depend on: it must see the very bare value
    require_dataset_identity refuses, or the script could never fix it."""
    ...
    document = read_dataset_identity_document(root)
    assert document["fingerprint"] == bare
...
def test_a_bare_fingerprint_that_no_longer_matches_the_recompute_refuses_and_reports(tmp_path):
    ...
    assert read_dataset_identity_document(root)["fingerprint"] == bare  # nothing was written
...
def test_restamp_reports_a_post_write_mismatch_instead_of_trusting_its_own_recompute(
    ...
    def _wrong_register_dataset(dataset_root: str, crop: str, project_root: str) -> dict:
        ...
        wrong = "v1:" + "0" * 16
        document = {**read_dataset_identity_document(Path(dataset_root)), "fingerprint": wrong}
