...
def test_compute_matches_basic():
    ...
    gt_boxes = [BBox(100, 100, 200, 200, 0)]
    pred_boxes = [
        PredBBox(105, 105, 195, 195, 0, confidence=0.9),  # good match → TP
        PredBBox(400, 400, 450, 450, 0, confidence=0.8),  # no match → FP
    ]
    result = compute_matches(gt_boxes, [], pred_boxes, [], iou_threshold=0.5)
    assert len(result["tp"]) == 1
    assert len(result["fp"]) == 1
    assert len(result["fn"]) == 0
    assert result["tp"][0]["class_id"] == 0
...
def test_compute_matches_fn():
    ...
    gt_boxes = [BBox(100, 100, 200, 200, 0)]
    result = compute_matches(gt_boxes, [], [], [])
...
def test_compute_matches_conf_filter():
    ...
    gt_boxes = [BBox(100, 100, 200, 200, 0)]
    pred_boxes = [PredBBox(100, 100, 200, 200, 0, confidence=0.1)]
    result = compute_matches(gt_boxes, [], pred_boxes, [], conf_threshold=0.5)
    assert len(result["tp"]) == 0
    assert len(result["fp"]) == 0
...
def test_compute_matches_class_mismatch():
    """GT class 0, pred class 1 → FN + FP."""
    gt_boxes = [BBox(100, 100, 200, 200, 0)]
    pred_boxes = [PredBBox(100, 100, 200, 200, 1, confidence=0.9)]
    result = compute_matches(gt_boxes, [], pred_boxes, [])
    assert len(result["tp"]) == 0
    assert len(result["fp"]) == 1
    assert len(result["fn"]) == 1
...
def test_compute_matches_polygon():
    ...
    pred_polys = [PredPolygon([(5, 5), (95, 5), (95, 95), (5, 95)], 0, confidence=0.85)]
    result = compute_matches([], gt_polys, [], pred_polys)
    assert len(result["tp"]) == 1
...
def test_compute_matches_iou_threshold():
    ...
    pred_boxes = [PredBBox(80, 80, 180, 180, 0, confidence=0.9)]
