...
def test_the_majority_marker_reports_the_specs_reading_not_the_gates_verdict(
    ...
    """One delivery, two answers. Catkin's majority phrase maps to its 95% crossing on a reading the
    breeders have not confirmed, while every measurement dimension behind the dates cleared the gate.
    The marker column must say the reading is unconfirmed even though nothing about this delivery is
    provisional in the gate's sense."""
...
def test_the_web_and_mcp_deliveries_agree_on_the_majority_marker(
    ...
    marker = phenology.majority_crossing_unconfirmed_column(get_trait("catkin"))
