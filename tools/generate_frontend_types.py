...
carries (hand-typed in ``client.ts``, never generated itself); ``routes/results.py`` declares
the phenology and count export request shapes; and
...
def declared_models() -> list[type[BaseModel]]:
    ...
    from tcip_web.routes.results import (
        AcknowledgmentPayload, ExportCountCsvPayload, ExportCsvPayload,
    ...
    return [GridGeometry, StatsSource, WorkingScale, CoverageViewing, CoverageRecord,
            ...
            RenamePreview, RenameRequest, RenameResponse, PhenologyPayload,
            ...
            DeliveryEventRecord, DeliverySupersessionRecord]
...
def _ts_type(schema: dict, where: str) -> str:
    ...
    if "anyOf" in schema or "oneOf" in schema:
        parts: list[str] = []
        for part in schema.get("anyOf") or schema["oneOf"]:
    ...
    if kind is None and not schema.keys() - {"default", "title", "description"}:
        return "unknown"  # a field carried uninterpreted (``Any``): the backend reads it
    if kind == "null":
        return "null"
...
def render() -> str:
    ...
    suffix_const = f"export const LABEL_SUFFIX = {json.dumps(label_suffix())};\n\n"
