...
| packages/tcip-web/frontend/src/api/http.ts | Shared fetch helpers. | 0 | 18 |
...
| packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx | (none found) | 33 | 2 |
...
| POST | `/compare/best` | `compare_best_route` | `routes/training.py:198` |
| GET | `/metric-directions` | `metric_directions_route` | `routes/training.py:249` |
| WS | `/runs/{run_id}/stream` (full path `/api/training/runs/{run_id}/stream`) | `training_stream_ws` | `routes/training.py:339` |
...
Side B: `packages/tcip-web/src/tcip_web/routes/training.py:299` (`read_log(key, after=cursor)`, the training stream's incremental tail, pushed as a `TrainingMetricFrame` per row) and `routes/tuning.py:599` (`read_log(trial_metrics_key`, answered in the shape `_metrics_common.metrics_response` builds).
...
Side A: `packages/tcip-web/src/tcip_web/routes/training.py:338` (`@router.websocket("/runs/{run_id}/stream")`).
...
Side B: `packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx:405` (`base_mtime: paths.mtime,`).
Phase 3 verdict: single.
