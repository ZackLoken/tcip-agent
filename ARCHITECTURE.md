...
| packages/tcip-mcp/src/tcip_mcp/pipelines/measurement/scale_calibration.py | Deriving and validating a physical per-pixel scale against real physical measurements. | 2 | 1 |
...
| packages/tcip-annotation/src/tcip_annotation/review_engine.py | ReviewEngine: review logic, detection walk-through, accept/reject. | 3 | 5 |
...
| packages/tcip-web/src/tcip_web/routes/results.py | Results routes: plant-mapping, per-plant phenology curves, CSV export, the operationalization record surface, the trait-spec statement surface, and the read-only delivery-event list. | 10 | 1 |
...
| packages/tcip-web/frontend/src/api/types.generated.ts | The coverage record's types, generated by scripts/generate_frontend_types.py from the pydantic models it renders (routes/_coverage_models.py, routes/coverage.py). | 0 | 7 |
...
| packages/tcip-web/frontend/src/store/index.ts | (none found) | 2 | 49 |  <!-- queued: P5-238 merge-or-split -->
...
| packages/tcip-web/frontend/src/store/types.ts | Types mirroring the Python backend's GuiState and the name-based label schema. | 1 | 27 |
...
| packages/tcip-web/frontend/src/tabs/ReviewTab.tsx | (none found) | 22 | 2 |  <!-- queued: P5-237 merge-or-split -->
...
## Modules with zero importers (99)
...
all 15 tool modules; ten of the sites read `@audited(scope_arg=...)`, so a check must match
...
| `stage_proposals` | `annotation_tools.py:785` | yes | Stage model-/agent-proposed shapes to ``predictions/<model>/<date>/<stem>.json`` for canvas |
| `write_class_map` | `annotation_tools.py:916` | yes | Author the dataset's nested class registry, a thin wrapper over ``class_registry``. |
...
| `prioritize_review_queue` | `feedback_tools.py:180` | yes | Order un-reviewed images for the next review batch. |  <!-- queued: P5-51 merge-or-split -->
...
| `run_inference` | `inference_tools.py:537` | yes | Run a trained model on images. |
| `export_predictions` | `inference_tools.py:1452` | yes | Run inference and save predictions as COCO/JSON prediction file(s). |  <!-- queued: P5-36 unify -->
| `tabulate_counts` | `inference_tools.py:1734` | yes | Run inference and export a CSV summary of detection counts per image. |  <!-- queued: P5-37 merge-or-split -->
...
| `launch_training` | `training_tools.py:367` | yes | Launch a training run in an isolated subprocess from a bespoke ``model_source`` builder. |
| `check_training_status` | `training_tools.py:574` | yes | Check the status of a training run. |
| `list_training_runs` | `training_tools.py:628` | yes | List all training runs in this session. |  <!-- queued: P5-29 unify -->
| `cancel_training` | `training_tools.py:654` | yes | Request graceful cancellation of a running training run. |
| `inspect_compute_resources` | `training_tools.py:683` | yes | Report the host's current compute headroom, a fact to reason with before launching |  <!-- queued: P5-31 demote-to-script -->
| `run_hpo` | `training_tools.py:1053` | yes | Run hyperparameter optimization on Ray Tune, training each trial for real. |
| `evaluate_model` | `training_tools.py:2003` | yes | Evaluate a trained checkpoint on a (held-out) dataset and write test_results.json. |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/load` | `load_classes` | `routes/classes.py:119` |
| POST | `/save` | `save_classes` | `routes/classes.py:165` |
| GET | `/image_status` | `get_image_status` | `routes/classes.py:292` |
| POST | `/image_status` | `set_image_status` | `routes/classes.py:302` |
| POST | `/image_status/bulk` | `set_image_status_bulk` | `routes/classes.py:333` |
| POST | `/image_status/derive` | `derive_image_status` | `routes/classes.py:363` |
...
| POST | `/per_plant_curves` | `per_plant_curves` | `routes/results.py:468` |  <!-- queued: P5-130 merge-or-split -->
| POST | `/onset_dates` | `onset_dates` | `routes/results.py:501` |  <!-- queued: P5-131 merge-or-split -->
| POST | `/export_csv` | `export_csv` | `routes/results.py:534` |
| GET | `/traits` | `list_traits` | `routes/results.py:952` |
| GET | `/operationalization` | `get_operationalization` | `routes/results.py:651` |
| GET | `/operationalizations` | `list_operationalizations` | `routes/results.py:667` |
| POST | `/operationalization/confirm` | `confirm_operationalization` | `routes/results.py:705` |
| GET | `/trait-spec-statement` | `get_trait_spec_statement` | `routes/results.py:807` |
| GET | `/trait-spec-statements` | `list_trait_spec_statements` | `routes/results.py:823` |
| POST | `/trait-spec-statement/confirm` | `confirm_trait_spec_statement` | `routes/results.py:865` |
| GET | `/delivery-events` | `list_delivery_events` | `routes/results.py:931` |
...
| POST | `/mark_complete` | `mark_complete` | `routes/review.py:654` |
| POST | `/backup_labels` | `backup_labels` | `routes/review.py:704` |
| POST | `/save_gt` | `save_gt` | `routes/review.py:724` |
| POST | `/validate_reference` | `validate_reference` | `routes/review.py:774` |
| GET | `/image_status` | `get_image_status` | `routes/review.py:1102` |
| GET | `/image_statuses` | `image_statuses` | `routes/review.py:1140` |
| GET | `/generation_conf` | `get_generation_conf` | `routes/review.py:1168` |
| POST | `/queue/launch` | `launch_priority_queue` | `routes/review.py:1288` |
| GET | `/queue/{job_id}` | `get_priority_queue_job` | `routes/review.py:1316` |
...
from `tcip_mcp.server` and calls it: `packages/tcip-mcp/src/tcip_mcp/server.py:69`
...
`packages/tcip-annotation/src/tcip_annotation/review_engine.py:799`;
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:926`.

A prediction record's `created_by` is one spelling, `resolution.prediction_producer`,
`packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:993`, so every checkpoint-backed writer
...
`packages/tcip-mcp/src/tcip_mcp/class_registry.py:210`.
...
`assign_class_ids`, `class_registry.py:305`, derives the training-time name-to-id map from this
...
`packages/tcip-web/src/tcip_web/routes/classes.py:302`; `set_image_status_bulk`,
`routes/classes.py:333`; `tcip_mcp.tools.data_tools._carry_confirmed_negatives`
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/datasets.py:454`; `_status_bucket_for`,
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:633`, imported by the web route module.

The token a Complete stores here is subject-scoped before it ever reaches a writer: `mark_complete`,
`packages/tcip-web/src/tcip_web/routes/review.py:654`, derives it from the GT file through
...
Writers: `_stamp_digest`, `packages/tcip-web/src/tcip_web/routes/classes.py:262`, called from
`set_image_status`/`set_image_status_bulk` at confirmation time; and
`tcip_mcp.class_registry.stamp_unstamped_confirmations`,
`packages/tcip-mcp/src/tcip_mcp/class_registry.py:223`, called by both registry writers,
`save_classes` (`packages/tcip-web/src/tcip_web/routes/classes.py:165`) and `write_class_map`
(`packages/tcip-mcp/src/tcip_mcp/tools/annotation_tools.py:916`), before the new registry lands.
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:758`, whose `only_unstamped` argument keeps
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:428`;
`region_completeness_digest_path`, `dataset_layout.py:466`; shape guard
`normalize_region_completeness_store`, `dataset_layout.py:506`.
...
the calibration-gate side are `region_completeness_path` (`dataset_layout.py:428`),
`normalize_region_completeness_store` (`dataset_layout.py:506`) and `status_bucket`
(`dataset_layout.py:530`). Gap: `test_block_calibration.py`'s
...
- `config.json` (`config_key`, `experiments.py:114`): written by `create_experiment`,
  ...
  `experiments.py:942`, and `compare_experiments`, `experiments.py:1084`.
- `status.json` (`status_key`, line 140): written by `create_experiment` (332), `update_status`
  ...
  (`experiments.py:538`), `resolve_experiment_dir_for_run` (`experiments.py:534`). `state` is
  terminal-locked once `"completed"`/`"failed"`.
- `lineage.json` (`lineage_key`, line 164): written by `create_experiment` (332) and
  `update_lineage`, `experiments.py:913`. Read by `get_experiment` (942) and
  `get_experiment_lineage`, `experiments.py:1128`.
- `artifacts.json` (`artifacts_key`, line 187): written by `create_experiment` (332) and
  `record_artifact`, `experiments.py:881`. Read by `get_experiment` (942).
- `metrics.jsonl` (`metrics_key`, line 254, append-only): written by
  `log_metrics`, `experiments.py:686`. Read by `read_metrics`, `experiments.py:673`, which
...
- `validations.jsonl` (`validations_key`, line 272, append-only): the claims earned against this
  run's evidence. Written only by the module-private `_append_validation`, `experiments.py:768`
  (no public raw appender; the storage seam's generic append remains reachable and is a stated
  residual), which refuses a row missing any of `_VALIDATION_FIELDS` (`experiments.py:722`),
  ...
  `experiments.py:803`, `find_validation`, `experiments.py:818` (matching rows by recomputed
  `validation_digest`, `experiments.py:758`), and included whole by `get_experiment` (942). The
...
`register_model_from_experiment` (`experiments.py:955`, reading whether the run's config carries
...
Writer: `write_sidecar`, `resolution.py:896`, the one write, under the stamp's own lock;
...
Reader: `read_operating_point_sidecar`, `resolution.py:1047`, which never raises: an unreadable
...
(`review_engine.py:738`); `save_review_state`, `review_engine.py:305`, flushes every shard.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:686` (`def log_metrics(`, the one writer; the trainer and the envelope hand rows to the context's epoch sink instead of opening the file).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:665` (`def is_confirmed_negative(`, the one membership predicate; `normalize_status_store` is the one store guard, called by `confirmed_negative_names` and the resolver's confirmations term instead of inline re-implementations).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:108` (`def image_root(`, with `annotation_root`/`prediction_root` and the dated dir calls built on them, plus `bucket_subject_date` at line 475 as the published inverse of `status_bucket`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:804` (`def label_filename(`, with `annotation_path`/`prediction_path` built on it).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/class_registry.py:305` (`def assign_class_ids(`, the one assignment, reached by the loader through `pipelines/data/datasets.py:120`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:428` (`def region_completeness_path(dataset_root: str | Path) -> Path:`).
...
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:719` (`def dataset_fingerprint(dataset_root: str | Path) -> str | None:`).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:1149` (`def read_split_manifest(`, the one path and parse beside the member's key constructor; the writer persists through the same key).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/operating_point.py:587` (`def resolve_operating_point(`, the calibrated regime; a caller-supplied `max_dets` earns a derivation label only by naming where it came from, and otherwise records itself as a caller override).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:158` (`DEFAULT_CONF = 0.5`, with `DEFAULT_NMS_IOU`, `DEFAULT_OVERLAP` and `DEFAULT_MAX_DETS` declared beside it).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:179` (`class ResolvedParam:`; `.value` raises at line 216, and `unvalidated_value`, line 224, is how a door reads an unvalidated number).
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:104` (`_ACCEPTED_REFERENCES`, whose geometry entry is built from `_GEOMETRY_REFERENCE_BY_SOURCE`, line 64, the one statement of which tile-size source earns which reference; `tile_size_source_of`, line 84, reads it back the other way).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/traits.py:228` (`def crops_yml_path(`, the one placement of `.github/skills/crops/crops.yml`, loaded once for every reader of it by `_crops_traits`, line 228).
Side B: `packages/tcip-mcp/src/tcip_mcp/traits.py:288` (`_spec_from_config` cross-checks each spec against `_crops_vocab`, line 241) and `scripts/verify_skill_traits.py:46` (`load_vocab` checks a skill's trait tokens through that same read, and refuses an empty vocabulary rather than reporting a clean skill).
Phase 3 verdict: single.
...
Side B: `packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx:586` (`base_mtime: paths.mtime,`).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/server.py:52` (`def list_registered_tools() -> list[str]:`).
