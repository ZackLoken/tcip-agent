...
| packages/tcip-mcp/src/tcip_mcp/pipelines/data/label_queries.py | The label-store and registry query library: reads a dataset's per-image JSON or assembled COCO labels, its ``classes.json`` registry, and its confirmed-negative image-status store. | 8 | 9 |
...
| packages/tcip-mcp/src/tcip_mcp/pipelines/data/split_construction.py | Constructing and persisting training splits from a data config, beside ``splits.py``. | 9 | 3 |
| packages/tcip-mcp/src/tcip_mcp/pipelines/data/splits.py | Group-aware, annotation-stratified train/val splitting. | 5 | 12 |
...
| packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py | Runtime parameter resolution, the "derive, don't pin" currency. | 8 | 29 |
...
| packages/tcip-mcp/src/tcip_mcp/tools/trait_spec_authoring_tools.py | The agent's two doors for a trait spec: `author_trait_spec` creates (or restates, when a spec exists with no statement), `update_trait_spec_fields` edits fields on one already on record; neither confirms. | 3 | 1 |
...
| packages/tcip-web/src/tcip_web/jobstore.py | Persistence + memory-cap helpers for the web's async job registries, plus `JobRegistry`, the shared dict-plus-lock live registry inference.py, tuning.py, review.py's priority queue and images.py's overview builds adopt. | 1 | 7 |
...
| scripts/conform_model_registry_paths.py | Wrap a project's model registry index to schema_version 2 and respell every entry's checkpoint_path relative to its scope root, relocating a moved or replaced checkpoint by content digest. | 1 | 0 |
...
| `tabulate_counts` | `inference_tools.py:1610` | yes | Export a CSV summary of detection counts per image, from a live run or a persisted bucket. |
...
| `compute_phenology` | `phenology_tools.py:542` | yes | Per-plant phenology milestones from classified predictions + a plant mapping. |  <!-- queued: P5-43 unify -->
...
| `overlay_reference_grid` | `vision_tools.py:941` | yes | Render image with a labeled reference-grid overlay for spatial referencing. |
...
|---|---|---|---|
| GET | `/api/state` | `get_state` | `app.py:220` |
| POST | `/api/state/tab` | `set_active_tab` | `app.py:229` |
| WS | `/ws/state` | `state_ws` | `app.py:237` |
| GET | `/health` | `health` | `app.py:331` |
| GET | `/` | `index` | `app.py:339` |
| POST | `/api/events/{panel}` | `post_panel_event` | `app.py:403` |
| WS | `/ws/panel/{panel}` | `panel_ws` | `app.py:454` |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/tree` | `get_dataset_tree` | `routes/dataset.py:149` |  <!-- queued: P5-83 unify -->
| POST | `/select` | `select_dataset` | `routes/dataset.py:229` |
| POST | `/nav` | `set_current_image` | `routes/dataset.py:345` |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/launch` | `launch_inference` | `routes/inference.py:429` |
| GET | `/jobs` | `list_jobs` | `routes/inference.py:528` |
...
`bind_startup_root` (`packages/tcip-web/src/tcip_web/app.py:92`) reads the workspace's
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:875`, so every checkpoint-backed writer
...
Writer: `write_sidecar`, `resolution.py:778`, the one write, under the stamp's own lock;
...
Reader: `read_operating_point_sidecar`, `resolution.py:929`, which never raises: an unreadable
...
Writer: `StateStore._flush_sync`, `tcip_web/state.py:266`, debounced 0.5s after `mutate`/`replace`, which
...
Reader: `StateStore.load_from_disk`, `tcip_web/state.py:279`.
...
`packages/tcip-web/src/tcip_web/app.py:177`, answers a route that raises it with 400 and the
...
Side B: `packages/tcip-web/src/tcip_web/app.py:426` (`if event.event_type == PANEL_EVENT_REVIEW_FOCUS:`).
...
Side A: `packages/tcip-web/src/tcip_web/app.py:236` (`@app.websocket("/ws/state")`).
Side B: `packages/tcip-web/src/tcip_web/state.py:195` (`def version(self) -> int:`, "Monotonic version, bumped on every state change.").
Phase 3 verdict: duplicated.
...
Side B: `packages/tcip-web/src/tcip_web/app.py:295` (`def _find_static_dir() -> Path:`).
Phase 3 verdict: duplicated.
...
Side B: `packages/tcip-web/src/tcip_web/app.py:236` (`@app.websocket("/ws/state")`, one of the endpoints not under the `/api` prefix).
...
Side B: `packages/tcip-web/src/tcip_web/app.py:239` (`if not origin_allowed(websocket.headers.get("origin"), websocket.scope):`).
Phase 3 verdict: single.
