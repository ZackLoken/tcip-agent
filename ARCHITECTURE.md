...
| packages/tcip-web/frontend/src/App.tsx | (none found) | 29 | 2 |
...
| packages/tcip-web/frontend/src/api/client.ts | Typed REST client for the tcip-web backend. | 8 | 41 |
...
| packages/tcip-web/frontend/src/api/http.ts | Shared fetch helpers. | 0 | 23 |
...
| packages/tcip-web/frontend/src/api/types.generated.ts | Types generated by scripts/generate_frontend_types.py from the pydantic models that declare them (routes/_coverage_models.py, routes/coverage.py, routes/review.py, tcip_web.state.GuiVocabulary, tcip_mcp.web_client, tcip_web.jobstore). | 0 | 20 |
...
| packages/tcip-web/frontend/src/hooks/useRegionCompleteness.ts | Wires the region-completeness store into the Annotate tab: fetches every subject's attestation record and saved-annotation count for the open raster, exposes the active subject's own complete/stale cells separate from every other subject's, and posts an explicit attest/unattest/re-attest write. | 3 | 3 |
...
| packages/tcip-web/frontend/src/store/types.ts | Types mirroring the Python backend's GuiState and the name-based label schema. | 1 | 43 |
...
| `validate_data_quality` | `data_tools.py:566` | yes | Run quality checks on a dataset (any supported annotation format). |  <!-- queued: P5-18 unify -->
| `make_splits` | `data_tools.py:720` | yes | Compute a leakage-free, annotation-stratified train/val/calibration split. |
...
| tool | line | audited | docstring first line |
|---|---|---|---|
| `preflight_config` | `training_tools.py:229` | yes | Validate a training configuration before launching. |
| `launch_training` | `training_tools.py:633` | yes | Launch a training run in an isolated subprocess from a bespoke ``model_source`` builder. |
| `check_training_status` | `training_tools.py:867` | yes | Check the status of a training run. |
| `list_training_runs` | `training_tools.py:1010` | yes | List every training run this platform can currently account for. |
| `cancel_training` | `training_tools.py:1240` | yes | Request graceful cancellation of a running training run. |
| `run_hpo` | `training_tools.py:1808` | yes | Run hyperparameter optimization on Ray Tune, training each trial for real. |
| `cancel_hpo` | `training_tools.py:2179` | yes | Request cooperative cancellation of a running HPO sweep. |
| `evaluate_model` | `training_tools.py:2707` | yes | Evaluate a trained checkpoint on a (held-out) dataset and write test_results.json. |
...
| POST | `/runs/{run_id}/cancel` | `cancel_run_route` | `routes/training.py:185` |
| POST | `/compare` | `compare_runs_route` | `routes/training.py:205` |
| POST | `/compare/best` | `compare_best_route` | `routes/training.py:219` |
| GET | `/metric-directions` | `metric_directions_route` | `routes/training.py:270` |
| WS | `/runs/{run_id}/stream` (full path `/api/training/runs/{run_id}/stream`) | `training_stream_ws` | `routes/training.py:360` |
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1029`.
...
`packages/tcip-mcp/src/tcip_mcp/tools/data_tools.py:963`, then applied by
`_apply_negative_carry(negative_carry, out_dir, subject)`
`packages/tcip-mcp/src/tcip_mcp/tools/data_tools.py:1149`).
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:784`, imported by the web route module.
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:923`, whose `only_unstamped` argument keeps
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:568`;
`region_completeness_digest_path`, `dataset_layout.py:607`; shape guard
`normalize_region_completeness_store`, `dataset_layout.py:662`.
...
the calibration-gate side are `region_completeness_path` (`dataset_layout.py:568`),
`normalize_region_completeness_store` (`dataset_layout.py:662`) and `status_bucket`
(`dataset_layout.py:696`). Gap: `test_block_calibration.py`'s
...
- `config.json` (`config_key`, `experiments.py:117`): written by `create_experiment`,
  ...
  (`def get_experiment(`), and `compare_experiments`, `experiments.py:1643`.
...
- `artifacts.json` (`artifacts_key`, line 187): written by `create_experiment` (397),
  ...
  `record_artifact`, `experiments.py:1215`. Read by `get_experiment` (1288).
...
- `validations.jsonl` (`validations_key`, line 272, append-only): the claims earned against this
  run's evidence. Written only by the module-private `_append_validation`, `experiments.py:1049`
  (no public raw appender; the storage seam's generic append remains reachable and is a stated
  residual), which refuses a row missing any of `_VALIDATION_FIELDS` (`experiments.py:996`),
...
Writer: `make_splits`, `data_tools.py:720`, when `output_path` is given or `materialize=True`.
...
only writes one through `make_splits`. `bind_manifest_stems` (`splits.py:561`) reads all three
sides for one capture date, its own date-narrowing arithmetic extracted into
`narrow_manifest_to_date` (`splits.py:427`), which the data picker's own counts (below) call over
...
`training_tools.manifest_compatibility` (`training_tools.py:186`) is every objection a launch
...
itself; `training_tools.list_split_choices` (`training_tools.py:1069`), the relaunch data
...
(`training_tools.py:210`), the same function the relaunch route's own launch build calls.
...
`cal_holdout_lock_key`, `packages/tcip-mcp/src/tcip_mcp/pipelines/data/splits.py:1076`, under the
scope root the split was drawn over (`cal_holdout_scope_root`).

Writer: `resolve_locked_cal_holdout_split`, `splits.py:1314`, locking on first draw for a given
...
Side A: `packages/tcip-mcp/src/tcip_mcp/web_client.py:212` (`VALID_PANELS = frozenset(`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:816` (`def is_confirmed_negative(`, the one membership predicate; `normalize_status_store` is the one store guard, called by `confirmed_negative_names` and the resolver's confirmations term instead of inline re-implementations).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:129` (`def image_root(`, with `annotation_root`/`prediction_root` and the dated dir calls built on them, plus `bucket_subject_date`, `dataset_layout.py:708`, as the published inverse of `status_bucket`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:969` (`def label_filename(`, with `annotation_path`/`prediction_path` built on it).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:1819` (`def read_split_manifest(`, the one path and parse beside the member's key constructor; the writer persists through the same key).
...
Side B: `packages/tcip-mcp/src/tcip_mcp/tools/training_tools.py:284` (`training_source = config.get("training_source")`).
Phase 3 verdict: duplicated.
...
Side A: `packages/tcip-web/src/tcip_web/routes/training.py:359` (`@router.websocket("/runs/{run_id}/stream")`).
