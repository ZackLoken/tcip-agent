...
Source: the module inventory `tools/build_module_inventory.py` produces, run at HEAD 8445473b.
...
HEAD 8445473b has 435 modules across the six scanned roots (144606 total lines):
...
| `clear_prediction_bucket` | `inference_tools.py:1646` | yes | Move a terminal experiment's own recorded prediction bucket into a dated archive under `predictions/.cleared/`, so the path re-publishes: the audited remedy `run_inference`'s own docstring and `delivery.md` name for a bucket the pointer lock has otherwise made unreachable a second time (a completed or failed experiment's bucket publishes once through the ordinary doors). |
| `deliver_per_image_counts` | `inference_tools.py:2556` | yes | Export a CSV summary of detection counts per image, from a live run or a persisted bucket. |
...
| POST | `/action` | `record_action` | `routes/review.py:785` |
| POST | `/mark_complete` | `mark_complete` | `routes/review.py:973` |
| POST | `/backup_labels` | `backup_labels` | `routes/review.py:1048` |
| GET | `/image_statuses` | `image_statuses` | `routes/review.py:1102` |
| GET | `/generation_conf` | `get_generation_conf` | `routes/review.py:1139` |
| POST | `/queue/launch` | `launch_priority_queue` | `routes/review.py:1317` |
| GET | `/queue/{job_id}` | `get_priority_queue_job` | `routes/review.py:1345` |
...
`packages/tcip-annotation/src/tcip_annotation/json_io.py:835`;
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1260`.
...
`packages/tcip-mcp/src/tcip_mcp/tools/data_tools.py:1076`).

Readers: `tcip_mcp.pipelines.data.label_queries.confirmed_negative_names`,
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/label_queries.py:570`; `_status_bucket_for`,
...
`packages/tcip-web/src/tcip_web/routes/review.py:973`, derives it from the GT file through
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1047`, whose `only_unstamped` argument keeps
...
Path/shape definition: `tcip_mcp.dataset_layout.view_coverage_path`, `dataset_layout.py:540`,
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:625`;
`region_completeness_digest_path`, `dataset_layout.py:664`; shape guard
`normalize_region_completeness_store`, `dataset_layout.py:756`.
...
the calibration-gate side are `region_completeness_path` (`dataset_layout.py:625`),
`normalize_region_completeness_store` (`dataset_layout.py:756`) and `status_bucket`
(`dataset_layout.py:790`). Gap: `test_block_calibration.py`'s
...
(`resolution.py:2464`, dataset-scoped when a
...
`packages/tcip-annotation/src/tcip_annotation/json_io.py:835` (format 1's writer).
...
`_resolve_writable_bucket_for`, `tools/inference_tools.py:1071`.
...
(`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:723`), built on
...
Path/shape definition: `tcip_mcp.dataset_layout.coverage_grid_zoom_path`, `dataset_layout.py:695`.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:129` (`def image_root(`, with `annotation_root`/`prediction_root` and the dated dir calls built on them, plus `bucket_subject_date`, `dataset_layout.py:802`, as the published inverse of `status_bucket`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1200` (`def label_filename(`, with `annotation_path`/`prediction_path` built on it).
...
Side A: `packages/tcip-annotation/src/tcip_annotation/json_io.py:417` (`def annotation_from_payload(`, the one conversion from a client payload to a record, with `json_io.py:471` (`_annotations_of`) the one parse back and `json_io.py:835` (`write_annotations`) the one writer).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:625` (`def region_completeness_path(dataset_root: str | Path) -> Path:`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:351` (`def dataset_identity_path(dataset_root: str | Path) -> Path:`).
