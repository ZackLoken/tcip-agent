...
| packages/tcip-mcp/src/tcip_mcp/tools/training_tools.py | Training MCP tools, config validation, launch training, HPO, status. | 31 | 9 |
...
| packages/tcip-web/frontend/src/store/index.ts | (none found) | 13 | 61 |
...
| packages/tcip-web/frontend/src/store/types.ts | Types mirroring the Python backend's GuiState and the name-based label schema. | 1 | 41 |
| packages/tcip-web/frontend/src/tabs/AnnotateTab.test.tsx | (none found) | 6 | 0 |
| packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx | (none found) | 31 | 2 |
...
| `make_splits` | `data_tools.py:452` | yes | Compute a leakage-free, annotation-stratified train/val/calibration split. |
...
| `tabulate_counts` | `inference_tools.py:1618` | yes | Export a CSV summary of detection counts per image, from a live run or a persisted bucket. |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/grid` | `get_grid` | `routes/coverage.py:93` |
| GET | `` (root) | `get_coverage` | `routes/coverage.py:144` |
| POST | `` (root) | `post_coverage` | `routes/coverage.py:190` |
| GET | `/completeness` | `get_completeness` | `routes/coverage.py:280` |
| POST | `/completeness` | `post_completeness` | `routes/coverage.py:325` |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/launch` | `launch_hpo` | `routes/tuning.py:316` |  <!-- queued: P5-113 delete -->
| GET | `/sweeps` | `list_sweeps` | `routes/tuning.py:343` |
| GET | `/sweeps/{sweep_id}` | `get_sweep` | `routes/tuning.py:359` |
| GET | `/sweeps/{sweep_id}/trials` | `list_trials` | `routes/tuning.py:391` |
| GET | `/sweeps/{sweep_id}/trials/{trial_id}/metrics` | `get_trial_metrics` | `routes/tuning.py:424` |
| GET | `/ray-dashboard` | `get_ray_dashboard` | `routes/tuning.py:451` |
| POST | `/sweeps/{sweep_id}/tensorboard` | `launch_sweep_tensorboard` | `routes/tuning.py:537` |
| POST | `/sweeps/{sweep_id}/trials/{trial_id}/tensorboard` | `launch_trial_tensorboard` | `routes/tuning.py:554` |
| POST | `/sweeps/{sweep_id}/trials/{trial_id}/tensorboard/stop` | `stop_trial_tensorboard` | `routes/tuning.py:568` |
...
`packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:255`.
...
`packages/tcip-mcp/src/tcip_mcp/tools/data_tools.py:892`).
...
- `status.json` (`status_key`, line 140): written by `create_experiment` (397), `update_status`,
  ...
  (1288), `reconstruct_run_status`, `experiments.py:810` (`def reconstruct_run_status(`),
...
- `artifacts.json` (`artifacts_key`, line 187): written by `create_experiment` (397),
  ...
  `record_artifact`, `experiments.py:1165`. Read by `get_experiment` (1288).
...
- `validations.jsonl` (`validations_key`, line 272, append-only): the claims earned against this
  run's evidence. Written only by the module-private `_append_validation`, `experiments.py:999`
...
Writer: `stage_prediction_shapes`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:255`, the
...
`resolve_prediction_bucket`, `prediction_buckets.py:224`, resolves a `(dataset_root, model_name,
date)` triple to a writable directory; `resolve_writable_bucket`, `prediction_buckets.py:192`,
redirects to the next free `<model_name>@r2`/`@r3` variant once any image in a bucket has a
recorded review verdict; `BucketHasVerdicts`, `prediction_buckets.py:158`, is raised instead when
`overwrite=True` is requested against a verdicted bucket.

Readers: `bucket_stems`, `prediction_buckets.py:23`, walks each dir through
...
excluded here too; `verdict_count`, `prediction_buckets.py:145`, delegates
...
Writer: `StateStore._flush_sync`, `tcip_web/state.py:285`, debounced 0.5s after `mutate`/`replace`, which
...
Reader: `StateStore.load_from_disk`, `tcip_web/state.py:298`.
...
`review_state_dir_of`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:111`;
`verdict_count`, `prediction_buckets.py:145`, opens a `ReviewEngine` on that root rather than
...
Writer: `make_splits`, `data_tools.py:452`, when `output_path` is given or `materialize=True`.
...
Phase 3 verdict: single. The current generation also rides the GuiState broadcast envelope (`packages/tcip-web/src/tcip_web/app.py:185` `SERVER_EPOCH`, read off `StateStore.binding_generation`, `packages/tcip-web/src/tcip_web/state.py:153`) and is adopted with the dataset in one client-side store update (`packages/tcip-web/frontend/src/store/slices/gui.ts:138` `applyRestoredDataset`, and `mergeSnapshot`), so the push's `binding_generation` and the reader's own comparison never straddle a stale identity.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:111` (`def review_state_dir_of(`, the one derivation of the store root; `verdict_count`, line 105, counts one bucket's verdicts through the store the engine writes into).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:192` (`def resolve_writable_bucket(`, the one guard; `bucket_stems`, `prediction_buckets.py:23`, excludes every provenance stamp through `tcip_annotation.json_io.prediction_documents` rather than naming one filename).
...
Side B: `packages/tcip-mcp/src/tcip_mcp/tools/training_tools.py:121` (`training_source = config.get("training_source")`).
Phase 3 verdict: duplicated.
...
Side B: `packages/tcip-annotation/src/tcip_annotation/sam_wrapper.py:329` (`def grid_to_rect(`, the one cell-name lookup, with `grid_to_pixel`, line 360, built on it) and `packages/tcip-web/src/tcip_web/routes/coverage.py:92` (`@router.get("/grid")`, `get_grid`, whose cell list the browser consumes verbatim).
Phase 3 verdict: single.
