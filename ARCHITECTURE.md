...
Source: the module inventory `tools/build_module_inventory.py` produces, run at HEAD 56fec587.
...
HEAD 56fec587 has 436 modules across the six scanned roots (145630 total lines):
...
| packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py | Runtime parameter resolution, the "derive, don't pin" currency. | 9 | 40 |
...
| packages/tcip-web/src/tcip_web/routes/projects.py | Workspace project discovery + the active-project marker + project removal. | 13 | 2 |
...
| tool | line | audited | docstring first line |
|---|---|---|---|
| `run_inference` | `inference_tools.py:291` | yes | Run a trained model over images or a raster, and persist the predictions as a bucket. |
| `clear_prediction_bucket` | `inference_tools.py:1658` | yes | Move a terminal experiment's own recorded prediction bucket into a dated archive under `predictions/.cleared/`, so the path re-publishes: the audited remedy `run_inference`'s own docstring and `delivery.md` name for a bucket the pointer lock has otherwise made unreachable a second time (a completed or failed experiment's bucket publishes once through the ordinary doors). |
...
| tool | line | audited | docstring first line |
|---|---|---|---|
| `redraw_calibration_holdout` | `calibration_tools.py:22` | yes | Deliberately redraw a locked calibration/holdout split. |
| `calibrate_scalar_operating_point` | `calibration_tools.py:258` | yes | Calibrate and validate a trait's ordinal-rank or continuous-value prediction against a |
| `calibrate_count_operating_point` | `calibration_tools.py:461` | yes | Calibrate and validate the count operating point against held-out GT, earning a claim |
...
| `deliver_phenology_milestones` | `phenology_tools.py:706` | yes | Per-plant phenology milestones from classified predictions + a plant mapping. |  <!-- queued: P5-43 unify -->
...
| `stage_proposals` | `proposal_tools.py:752` | yes | Stage model-/agent-proposed shapes as predictions for canvas review, the "show on canvas |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/sweeps` | `relaunch_sweep` | `routes/tuning.py:567` |
| POST | `/sweeps/{sweep_id}/cancel` | `cancel_sweep_route` | `routes/tuning.py:606` |
| GET | `/sweeps` | `list_sweeps` | `routes/tuning.py:617` |
| GET | `/sweeps/{sweep_id}` | `get_sweep` | `routes/tuning.py:631` |
| GET | `/sweeps/{sweep_id}/trials` | `list_trials` | `routes/tuning.py:672` |
| GET | `/sweeps/{sweep_id}/trials/{trial_id}/metrics` | `get_trial_metrics` | `routes/tuning.py:693` |
| GET | `/ray-dashboard` | `get_ray_dashboard` | `routes/tuning.py:720` |
| POST | `/sweeps/{sweep_id}/tensorboard` | `launch_sweep_tensorboard` | `routes/tuning.py:806` |
| POST | `/sweeps/{sweep_id}/trials/{trial_id}/tensorboard` | `launch_trial_tensorboard` | `routes/tuning.py:823` |
| POST | `/sweeps/{sweep_id}/trials/{trial_id}/tensorboard/stop` | `stop_trial_tensorboard` | `routes/tuning.py:837` |
...
`packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:443`.
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1266`.
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:884`, imported by the web route module.
...
`tcip_mcp.subject_registry._sweep_schema_change`,
`packages/tcip-mcp/src/tcip_mcp/subject_registry.py:329`, called through `replace_registry`
(`packages/tcip-mcp/src/tcip_mcp/subject_registry.py:423`) by both registry writers,
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1053`, whose `only_unstamped` argument keeps
...
`attribute_schema_digest`, `subject_registry.py:195`. Both sides run their real implementations
...
Path/shape definition: `tcip_mcp.dataset_layout.view_coverage_path`, `dataset_layout.py:546`,
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:631`;
`region_completeness_digest_path`, `dataset_layout.py:670`; shape guard
`normalize_region_completeness_store`, `dataset_layout.py:762`.
...
the calibration-gate side are `region_completeness_path` (`dataset_layout.py:631`),
`normalize_region_completeness_store` (`dataset_layout.py:762`) and `status_bucket`
(`dataset_layout.py:796`). Gap: `test_block_calibration.py`'s
...
`tools/annotation_tools.py:513`; `redraw_calibration_holdout`, `tools/calibration_tools.py:22`;
`materialize_review_dataset`, `tools/feedback_tools.py:167`; `run_inference`,
`tools/inference_tools.py:291`; `register_dataset`, `tools/project_tools.py:189`;
...
`calibration_tools.py`'s redraw event (`redraw_calibration_holdout_result`, `tools/calibration_tools.py:214`).
...
Writer: `stage_prediction_shapes`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:443`, the
...
`resolve_prediction_bucket`, `prediction_buckets.py:409`, resolves a `(dataset_root, model_name,
...
`_resolve_writable_bucket_for`, `tools/inference_tools.py:1083`.
...
`review_state_dir_of`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:142`;
`verdict_count`, `prediction_buckets.py:191`, opens a `ReviewEngine` on that root rather than
...
(`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:729`), built on
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:82` (`_STATE_DOC` locator), `frozen: false`, the
same declared classification `view_coverage.json` carries.

Path/shape definition: `tcip_mcp.dataset_layout.coverage_grid_zoom_path`, `dataset_layout.py:701`.
...
arguments: dict) -> None:`, which calls `record_committed` (`review.py:110`, `routes/audit_gap.py`)
...
Side B: `packages/tcip-web/src/tcip_web/routes/training.py:334` (`asyncio.to_thread(read_log, key, after=cursor)`, the training stream's incremental tail read off the event loop, pushed as a `TrainingMetricFrame` per row) and `routes/tuning.py:706` (`read_log(trial_metrics_key`, answered in the shape `_metrics_common.metrics_response` builds).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:135` (`def image_root(`, with `annotation_root`/`prediction_root` and the dated dir calls built on them, plus `bucket_subject_date`, `dataset_layout.py:808`, as the published inverse of `status_bucket`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1206` (`def label_filename(`, with `annotation_path`/`prediction_path` built on it).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:142` (`def review_state_dir_of(`, the one derivation of the store root; `verdict_count`, `prediction_buckets.py:191`, counts one bucket's verdicts through the store the engine writes into).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:631` (`def region_completeness_path(dataset_root: str | Path) -> Path:`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:357` (`def dataset_identity_path(dataset_root: str | Path) -> Path:`).
...
Side B: `packages/tcip-web/frontend/src/components/TerminalRail.tsx:313` (`send({ type: "input", data });`).
Phase 3 verdict: duplicated.
...
Side A: `packages/tcip-web/src/tcip_web/paths.py:55` (`def allowed_roots() -> list[Path]:`).
