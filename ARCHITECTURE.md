...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:2665` (`def check_delivery_gate(`, judging each dimension against `_DIMENSION_REFERENCES`, `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:2645`, so a reference clears only the dimension whose kind earned it, with `DeliveryGateResult.column_stamp`, `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:2602`, as the one derivation of what a deliverable's validity column carries).
