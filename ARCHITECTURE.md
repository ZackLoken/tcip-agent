...
| `export_predictions` | `inference_tools.py:1421` | yes | Run inference and save predictions as COCO/JSON prediction file(s). |  <!-- queued: P5-36 unify -->
| `tabulate_counts` | `inference_tools.py:1693` | yes | Run inference and export a CSV summary of detection counts per image. |  <!-- queued: P5-37 merge-or-split -->
...
| `launch_training` | `training_tools.py:357` | yes | Launch a training run in an isolated subprocess from a bespoke ``model_source`` builder. |
| `check_training_status` | `training_tools.py:562` | yes | Check the status of a training run. |
| `list_training_runs` | `training_tools.py:616` | yes | List all training runs in this session. |  <!-- queued: P5-29 unify -->
| `cancel_training` | `training_tools.py:642` | yes | Request graceful cancellation of a running training run. |
| `inspect_compute_resources` | `training_tools.py:671` | yes | Report the host's current compute headroom, a fact to reason with before launching |  <!-- queued: P5-31 demote-to-script -->
| `run_hpo` | `training_tools.py:1021` | yes | Run hyperparameter optimization on Ray Tune, training each trial for real. |
| `evaluate_model` | `training_tools.py:1952` | yes | Evaluate a trained checkpoint on a (held-out) dataset and write test_results.json. |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/load` | `load_classes` | `routes/classes.py:123` |
| POST | `/save` | `save_classes` | `routes/classes.py:169` |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/launch` | `launch_inference` | `routes/inference.py:404` |  <!-- queued: P5-105 delete -->
| GET | `/jobs` | `list_jobs` | `routes/inference.py:504` |
| GET | `/jobs/{job_id}` | `get_job` | `routes/inference.py:522` |
| GET | `/jobs/{job_id}/preview` | `get_preview` | `routes/inference.py:539` |
| POST | `/jobs/{job_id}/cancel` | `cancel_job` | `routes/inference.py:553` |
| WS | `/jobs/{job_id}/stream` | `stream_job` | `routes/inference.py:563` |
...
| POST | `/per_plant_curves` | `per_plant_curves` | `routes/results.py:464` |  <!-- queued: P5-130 merge-or-split -->
| POST | `/onset_dates` | `onset_dates` | `routes/results.py:497` |  <!-- queued: P5-131 merge-or-split -->
| POST | `/export_csv` | `export_csv` | `routes/results.py:530` |
| GET | `/traits` | `list_traits` | `routes/results.py:975` |
| GET | `/operationalization` | `get_operationalization` | `routes/results.py:674` |
| GET | `/operationalizations` | `list_operationalizations` | `routes/results.py:690` |
| POST | `/operationalization/confirm` | `confirm_operationalization` | `routes/results.py:728` |
| GET | `/trait-spec-statement` | `get_trait_spec_statement` | `routes/results.py:830` |
| GET | `/trait-spec-statements` | `list_trait_spec_statements` | `routes/results.py:846` |
| POST | `/trait-spec-statement/confirm` | `confirm_trait_spec_statement` | `routes/results.py:888` |
| GET | `/delivery-events` | `list_delivery_events` | `routes/results.py:954` |
...
`packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:237`.
...
`routes/classes.py:337`; `tcip_mcp.tools.data_tools._carry_confirmed_negatives`
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/datasets.py:444`; `_status_bucket_for`,
...
- `config.json` (`config_key`, `experiments.py:114`): written by `create_experiment`,
  `experiments.py:332`, and `overwrite_config_if_pristine`, `experiments.py:416` (rewrites only
  while the record is still pristine, no metrics logged). Read by `get_experiment`,
  `experiments.py:942`, and `compare_experiments`, `experiments.py:1046`.
- `status.json` (`status_key`, line 140): written by `create_experiment` (332), `update_status`
  (`experiments.py:413`), `stamp_run_identity` (`experiments.py:495`), `_touch_heartbeat`
  (`experiments.py:593`). Read by `get_experiment` (942), `reconstruct_run_status`
  (`experiments.py:538`), `resolve_experiment_dir_for_run` (`experiments.py:519`). `state` is
  terminal-locked once `"completed"`/`"failed"`.
- `lineage.json` (`lineage_key`, line 164): written by `create_experiment` (332) and
  `update_lineage`, `experiments.py:893`. Read by `get_experiment` (942) and
  `get_experiment_lineage`, `experiments.py:1090`.
- `artifacts.json` (`artifacts_key`, line 187): written by `create_experiment` (332) and
  `record_artifact`, `experiments.py:861`. Read by `get_experiment` (942).
- `metrics.jsonl` (`metrics_key`, line 254, append-only): written by
  `log_metrics`, `experiments.py:671`. Read by `read_metrics`, `experiments.py:658`, which
...
- `validations.jsonl` (`validations_key`, line 272, append-only): the claims earned against this
  run's evidence. Written only by the module-private `_append_validation`, `experiments.py:748`
  (no public raw appender; the storage seam's generic append remains reachable and is a stated
  residual). Read by `read_validations`, `experiments.py:783`, `find_validation`,
  `experiments.py:750` (matching rows by recomputed `validation_digest`, `experiments.py:738`),
...
Writer: `stage_prediction_shapes`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:237`, the
...
`resolve_prediction_bucket`, `prediction_buckets.py:206`, resolves a `(dataset_root, model_name,
date)` triple to a writable directory; `resolve_writable_bucket`, `prediction_buckets.py:174`,
redirects to the next free `<model_name>@r2`/`@r3` variant once any image in a bucket has a
recorded review verdict; `BucketHasVerdicts`, `prediction_buckets.py:140`, is raised instead when
...
measurement dimension is excluded here too; `verdict_count`, `prediction_buckets.py:127`, delegates
...
Writer: `write_sidecar`, `resolution.py:868`, the one write, under the stamp's own lock;
...
Reader: `read_operating_point_sidecar`, `resolution.py:998`, which never raises: an unreadable
...
`review_state_dir_of`, `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:93`;
`verdict_count`, `prediction_buckets.py:127`, opens a `ReviewEngine` on that root rather than
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:671` (`def log_metrics(`, the one writer; the trainer and the envelope hand rows to the context's epoch sink instead of opening the file).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:93` (`def review_state_dir_of(`, the one derivation of the store root; `verdict_count`, line 105, counts one bucket's verdicts through the store the engine writes into).
...
Side B: `packages/tcip-web/src/tcip_web/routes/classes.py:182` (`from tcip_mcp.dataset_layout import classes_path`).
Phase 3 verdict: single.
...
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:692` (`def dataset_fingerprint(dataset_root: str | Path) -> str | None:`).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/prediction_buckets.py:174` (`def resolve_writable_bucket(`, the one guard; `bucket_stems`, line 20, excludes every provenance stamp through `resolution.SIDECAR_FILENAMES` rather than naming one filename).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:1111` (`def read_split_manifest(`, the one path and parse beside the member's key constructor; the writer persists through the same key).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:172` (`class ResolvedParam:`; `.value` raises at line 216, and `unvalidated_value`, line 224, is how a door reads an unvalidated number).
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:97` (`_ACCEPTED_REFERENCES`, whose geometry entry is built from `_GEOMETRY_REFERENCE_BY_SOURCE`, line 64, the one statement of which tile-size source earns which reference; `tile_size_source_of`, line 84, reads it back the other way).
...
Side B: `packages/tcip-mcp/src/tcip_mcp/traits.py:268` (`_spec_from_config` cross-checks each spec against `_crops_vocab`, line 241) and `scripts/verify_skill_traits.py:46` (`load_vocab` checks a skill's trait tokens through that same read, and refuses an empty vocabulary rather than reporting a clean skill).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/data/datasets.py:1486` (`DATASET_SOURCE_KEY = "dataset_source"`).
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/data/datasets.py:1600` (`def build_dataset(task: str, dataset_source: dict | None = None, **kwargs) -> Dataset:`).
Phase 3 verdict: duplicated.
...
Side A: `packages/tcip-web/src/tcip_web/routes/inference.py:562` (`@router.websocket("/jobs/{job_id}/stream")`).
