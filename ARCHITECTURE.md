...
| packages/tcip-mcp/src/tcip_mcp/dataset_layout.py | Canonical dataset-layout resolver: the single source of truth for where an image's ground-truth labels and model predictions live on disk. | 7 | 49 |
...
| packages/tcip-mcp/src/tcip_mcp/tools/project_tools.py | Project management tools. | 19 | 14 |
...
| Module path | Ownership (one line) | In-repo imports | Imported by |
|---|---|---|---|
| packages/tcip-store/src/tcip_store/__init__.py | The storage seam's public surface: keys, errors, store declarations, and the module-level operations. | 6 | 76 |
...
| packages/tcip-store/src/tcip_store/binding.py | Which backend a process binds at its entry point: the database unless `TCIP_STORE_BACKEND` names the file backend, and a refusal for any other name. | 3 | 40 |
...
| packages/tcip-web/frontend/src/components/annotate/AnnotateLegend.tsx | Hover-triggered legend anchored lower-left of the Annotate canvas: one row per dataset subject plus the selected-shape colour, and in box mode an extra row explaining the dashed derived-box outline. | 3 | 1 |
...
| packages/tcip-web/frontend/src/lib/subjectColors.test.ts | (none found) | 0 | 0 |
...
| packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx | (none found) | 35 | 2 |
...
| `deliver_per_image_counts` | `inference_tools.py:1914` | yes | Export a CSV summary of detection counts per image, from a live run or a persisted bucket. |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/labels` | `load_labels` | `routes/annotate.py:175` |
| POST | `/labels` | `save_labels` | `routes/annotate.py:199` |
...
| GET | `/bands` | `get_bands` | `routes/images.py:717` |
| POST | `/overviews` | `build_image_overviews` | `routes/images.py:854` |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/launch` | `launch_inference` | `routes/inference.py:417` |
| GET | `/jobs` | `list_jobs` | `routes/inference.py:517` |
| POST | `/jobs/{job_id}/cancel` | `cancel_job` | `routes/inference.py:521` |
| WS | `/jobs/{job_id}/stream` | `stream_job` | `routes/inference.py:531` |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/matches` | `compute_image_matches` | `routes/review.py:430` |
| POST | `/action` | `record_action` | `routes/review.py:556` |
| POST | `/mark_complete` | `mark_complete` | `routes/review.py:692` |
| POST | `/backup_labels` | `backup_labels` | `routes/review.py:754` |
| GET | `/image_statuses` | `image_statuses` | `routes/review.py:808` |
| GET | `/generation_conf` | `get_generation_conf` | `routes/review.py:838` |
...
`packages/tcip-web/src/tcip_web/routes/review.py:692`, derives it from the GT file through
...
Side A: `packages/tcip-web/src/tcip_web/routes/annotate.py:42` (`bbox: Optional[list[float]] = None          # [x1, y1, x2, y2], pixel`, the wire form).
...
Side A: `packages/tcip-web/src/tcip_web/routes/inference.py:530` (`@router.websocket("/jobs/{job_id}/stream")`).
...
Side A: `packages/tcip-web/src/tcip_web/routes/images.py:695` (`"X-TCIP-Stats-Source": json.dumps(stats_source.model_dump(), allow_nan=False),`).
...
Side A: `packages/tcip-web/src/tcip_web/routes/annotate.py:194` (`"base_mtime": token,`, the token the load route mints; the save route compares the echoed one at `routes/annotate.py:200`).
...
Side B: `packages/tcip-web/src/tcip_web/routes/annotate.py:82` (`p = assert_path_allowed(path)`).
Phase 3 verdict: single.
