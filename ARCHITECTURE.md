...
| packages/tcip-mcp/src/tcip_mcp/pipelines/calibration.py | The calibrate/summarize pair every inference entry point shares: resolve a per-dataset operating point from a labeled split, and its compact, response-safe gate-evidence summary. | 10 | 1 |
...
| packages/tcip-mcp/src/tcip_mcp/pipelines/derivations.py | Tier-A data/model derivations, read the artifact in hand, compute the value. | 7 | 11 |
...
| packages/tcip-mcp/src/tcip_mcp/pipelines/training/generic_trainer.py | Task-agnostic training loop for a bespoke ``model_source`` model. | 12 | 7 |
...
| packages/tcip-mcp/src/tcip_mcp/server.py | MCP server entry point: register all domain tools and run on stdio. | 23 | 22 |
...
| packages/tcip-mcp/src/tcip_mcp/tools/data_tools.py | Data management tools: load datasets, validate quality, split data. | 14 | 10 |
...
| packages/tcip-mcp/src/tcip_mcp/traits.py | Trait knowledge, the human-defined *semantics* of each measurable trait (Tier C). | 6 | 22 |
...
| packages/tcip-store/src/tcip_store/sqlite_backend.py | The database backend: one verified WAL database per root for records and logs, checked against each kind of root it serves on that kind's first use, with blobs forwarded to the file backend. | 5 | 5 |
...
| packages/tcip-web/src/tcip_web/paths.py | Path resolution helpers with traversal protection. | 4 | 13 |
...
| packages/tcip-web/src/tcip_web/routes/projects.py | Workspace project discovery + the active-project marker. | 5 | 1 |
| packages/tcip-web/src/tcip_web/routes/results.py | Results routes: plant-mapping, per-plant phenology curves, CSV export, the operationalization record surface, the trait-spec statement surface, and the read-only delivery-event list. | 18 | 1 |
...
| packages/tcip-web/frontend/src/api/classes.ts | Dataset class-registry + per-image-status API helpers. | 2 | 18 |
| packages/tcip-web/frontend/src/api/client.test.ts | (none found) | 3 | 0 |
| packages/tcip-web/frontend/src/api/client.ts | Typed REST client for the tcip-web backend. | 7 | 40 |
...
| packages/tcip-web/frontend/src/api/http.ts | Shared fetch helpers. | 0 | 21 |
...
| packages/tcip-web/frontend/src/api/training.ts | Training-tab specific REST + WebSocket helpers. | 4 | 11 |
...
| packages/tcip-web/frontend/src/components/TopBar.tsx | (none found) | 3 | 2 |
...
| packages/tcip-web/frontend/src/components/review/ReviewOverlays.tsx | Memoized layer drawing every review detection by its own geometry kind (point, box or polygon), coloured by outcome and highlighting the focused detection. | 7 | 1 |
...
| packages/tcip-web/frontend/src/hooks/useRegionCompleteness.ts | Wires the region-completeness store into the Annotate tab: fetches every subject's attestation record and saved-annotation count for the open raster, exposes the active subject's own complete/stale cells separate from every other subject's, and posts an explicit attest/unattest/re-attest write. | 2 | 3 |
...
| packages/tcip-web/frontend/src/lib/reviewGeometry.ts | The single source of a review detection's geometry. | 1 | 3 |
...
| packages/tcip-web/frontend/src/store/index.ts | (none found) | 13 | 63 |
...
| packages/tcip-web/frontend/src/store/types.ts | Types mirroring the Python backend's GuiState and the name-based label schema. | 1 | 42 |
| packages/tcip-web/frontend/src/tabs/AnnotateTab.test.tsx | (none found) | 6 | 0 |
...
| packages/tcip-web/frontend/src/tabs/TrainingTab.tsx | (none found) | 13 | 2 |
| packages/tcip-web/frontend/src/tabs/TuningTab.test.tsx | (none found) | 4 | 0 |
| packages/tcip-web/frontend/src/tabs/TuningTab.tsx | (none found) | 12 | 2 |
...
| scripts/calibrate_operating_point.py | Calibrate + held-out validate a detection operating point over a labeled split. | 11 | 0 |
...
| scripts/conform_project_site.py | Write or correct one project's authored site: the record ``init_project``/``ingest_images`` themselves cannot reach for a project whose name does not fit the workspace scheme, and the one deliberate overwrite for a site typed wrong once or a record damaged by hand. | 3 | 0 |
...
## Modules with zero importers (131)
...
| `tabulate_counts` | `inference_tools.py:1621` | yes | Export a CSV summary of detection counts per image, from a live run or a persisted bucket. |
...
| method | path | handler | line |
|---|---|---|---|
| POST | `/state` | `push_canvas_state` | `routes/canvas.py:79` |
...
| method | path | handler | line |
|---|---|---|---|
| GET | `/grid` | `get_grid` | `routes/coverage.py:128` |
| GET | `` (root) | `get_coverage` | `routes/coverage.py:170` |
| POST | `` (root) | `post_coverage` | `routes/coverage.py:216` |
| GET | `/completeness` | `get_completeness` | `routes/coverage.py:312` |
| POST | `/completeness` | `post_completeness` | `routes/coverage.py:391` |

### routes/dataset.py, prefix `/api/dataset` (3 routes)

| method | path | handler | line |
|---|---|---|---|
| GET | `/tree` | `get_dataset_tree` | `routes/dataset.py:150` |  <!-- queued: P5-83 unify -->
| POST | `/select` | `select_dataset` | `routes/dataset.py:230` |
| POST | `/nav` | `set_current_image` | `routes/dataset.py:346` |
...
| GET | `/overviews/status` | `get_overview_job` | `routes/images.py:870` |
...
| POST | `/runs/{run_id}/cancel` | `cancel_run_route` | `routes/training.py:170` |
| POST | `/compare` | `compare_runs_route` | `routes/training.py:190` |
| POST | `/compare/best` | `compare_best_route` | `routes/training.py:204` |
| GET | `/metric-directions` | `metric_directions_route` | `routes/training.py:255` |
| WS | `/runs/{run_id}/stream` (full path `/api/training/runs/{run_id}/stream`) | `training_stream_ws` | `routes/training.py:345` |
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:1027`.

A prediction record's `created_by` is one spelling, `resolution.prediction_producer`,
`packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:884`, so every checkpoint-backed writer
...
`packages/tcip-annotation/src/tcip_annotation/format_io.py:279`;
...
Readers: `tcip_annotation.format_io.parse_coco_annotations`, `format_io.py:230`;
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/label_queries.py:91`
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:921`, whose `only_unstamped` argument keeps
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/label_queries.py:572`
...
`packages/tcip-mcp/src/tcip_mcp/pipelines/data/label_queries.py:344`
...
Path/shape definition: `tcip_mcp.dataset_layout.view_coverage_path`, `dataset_layout.py:483`,
...
`packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:566`;
`region_completeness_digest_path`, `dataset_layout.py:605`; shape guard
`normalize_region_completeness_store`, `dataset_layout.py:660`.
...
the calibration-gate side are `region_completeness_path` (`dataset_layout.py:566`),
`normalize_region_completeness_store` (`dataset_layout.py:660`) and `status_bucket`
(`dataset_layout.py:694`). Gap: `test_block_calibration.py`'s
...
`packages/tcip-mcp/src/tcip_mcp/experiments.py:1495`, one scan of the platform audit log
...
- `config.json` (`config_key`, `experiments.py:116`): written by `create_experiment`,
  `experiments.py:407` (`def create_experiment(`), `overwrite_config_if_pristine`,
  ...
  113 (`def _patch_experiment_config_split(`). Read by `get_experiment`, `experiments.py:1421`
  (`def get_experiment(`), and `compare_experiments`, `experiments.py:1595`.
- `status.json` (`status_key`, line 140): written by `create_experiment` (397), `update_status`,
  ...
  `_touch_heartbeat`, `experiments.py:865` (`def _touch_heartbeat(`). Read by `get_experiment`
  (1288), `reconstruct_run_status`, `experiments.py:812` (`def reconstruct_run_status(`),
  `resolve_experiment_dir_for_run` (`experiments.py:691`). `state` is terminal-locked once
  `"completed"`/`"failed"`.
- `lineage.json` (`lineage_key`, line 164): written by `create_experiment` (397), `complete_run`,
  `experiments.py:601` (`def complete_run(`, the run's own `model_weights`/`model_weights_sha256`
...
- `artifacts.json` (`artifacts_key`, line 187): written by `create_experiment` (397),
  ...
  `record_artifact`, `experiments.py:1167`. Read by `get_experiment` (1288).
- `metrics.jsonl` (`metrics_key`, line 254, append-only): written by
  `log_metrics`, `experiments.py:900`. Read by `read_metrics`, `experiments.py:884`, which
...
- `validations.jsonl` (`validations_key`, line 272, append-only): the claims earned against this
  run's evidence. Written only by the module-private `_append_validation`, `experiments.py:1001`
  (no public raw appender; the storage seam's generic append remains reachable and is a stated
  residual), which refuses a row missing any of `_VALIDATION_FIELDS` (`experiments.py:948`),
...
Reader: `read_operating_point_sidecar`, `resolution.py:947`, which never raises: an unreadable
...
Writer: `StateStore._flush_sync`, `tcip_web/state.py:286`, debounced 0.5s after `mutate`/`replace`, which
...
Reader: `StateStore.load_from_disk`, `tcip_web/state.py:299`.

`StateStore.mutate`, `tcip_web/state.py:31`, validates the merged mutation through `GuiState`
before holding it, raising `GuiMutationInvalid` (`tcip_web/state.py:30`) on a field that does not
...
`cal_holdout_lock_key`, `packages/tcip-mcp/src/tcip_mcp/pipelines/data/splits.py:966`, under the
scope root the split was drawn over (`cal_holdout_scope_root`).

Writer: `resolve_locked_cal_holdout_split`, `splits.py:1205`, locking on first draw for a given
...
Phase 3 verdict: single. The current generation also rides the GuiState broadcast envelope (`packages/tcip-web/src/tcip_web/app.py:185` `SERVER_EPOCH`, read off `StateStore.binding_generation`, `packages/tcip-web/src/tcip_web/state.py:154`) and is adopted with the dataset in one client-side store update (`packages/tcip-web/frontend/src/store/slices/gui.ts:138` `applyRestoredDataset`, and `mergeSnapshot`), so the push's `binding_generation` and the reader's own comparison never straddle a stale identity.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:814` (`def is_confirmed_negative(`, the one membership predicate; `normalize_status_store` is the one store guard, called by `confirmed_negative_names` and the resolver's confirmations term instead of inline re-implementations).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:129` (`def image_root(`, with `annotation_root`/`prediction_root` and the dated dir calls built on them, plus `bucket_subject_date`, `dataset_layout.py:706`, as the published inverse of `status_bucket`).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/dataset_layout.py:967` (`def label_filename(`, with `annotation_path`/`prediction_path` built on it).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/experiments.py:1771` (`def read_split_manifest(`, the one path and parse beside the member's key constructor; the writer persists through the same key).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:180` (`class ResolvedParam:`; `.value` raises at line 216, and `unvalidated_value`, line 224, is how a door reads an unvalidated number).
Side B: `packages/tcip-mcp/src/tcip_mcp/pipelines/resolution.py:105` (`_ACCEPTED_REFERENCES`, whose geometry entry is built from `_GEOMETRY_REFERENCE_BY_SOURCE`, line 64, the one statement of which tile-size source earns which reference; `tile_size_source_of`, line 84, reads it back the other way).
...
Side A: `packages/tcip-mcp/src/tcip_mcp/traits.py:353` (`def trait_specs_dir(`, the one placement, with `TRAIT_SPECS_STORE`, `traits.py:394`, and `trait_spec_key`, `traits.py:410`, addressing one spec).
Side B: `packages/tcip-mcp/src/tcip_mcp/traits.py:428` (`load_trait_specs_with_errors`, the one scan and the one skip-reason list) and `traits.py:481` (`write_trait_spec_fields`, the one write, reading and merging compare-and-set against the version it read). `packages/tcip-web/src/tcip_web/routes/results.py:443` and `scripts/doctor.py:211` name the project and let the placement resolve here.
Phase 3 verdict: single.
...
Side B: `packages/tcip-web/src/tcip_web/state.py:215` (`def version(self) -> int:`, "Monotonic version, bumped on every state change.").
Phase 3 verdict: duplicated.
...
Side A: `packages/tcip-web/src/tcip_web/routes/training.py:344` (`@router.websocket("/runs/{run_id}/stream")`).
...
Side B: `packages/tcip-web/frontend/src/tabs/AnnotateTab.tsx:406` (`base_mtime: paths.mtime,`).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/web_client.py:195` (`ActiveTab = Literal["annotate", "review", "training", "tuning", "inference", "results", "meta"]`, with `TAB_NAMES = get_args(ActiveTab)` beside it, `tcip_web.state` importing both).
...
Side B: `packages/tcip-annotation/src/tcip_annotation/sam_wrapper.py:329` (`def grid_to_rect(`, the one cell-name lookup, with `grid_to_pixel`, line 360, built on it) and `packages/tcip-web/src/tcip_web/routes/coverage.py:127` (`@router.get("/grid")`, `get_grid`, whose cell list the browser consumes verbatim).
Phase 3 verdict: single.
...
Side A: `packages/tcip-mcp/src/tcip_mcp/server.py:40` (`def list_registered_tools() -> list[str]:`).
